...
class Sequence implements Comparable<Sequence> {
	...
	static Map<String,String>	amimap = Map<String,String>();
	static Map<String,String>	revcom = Map<String,String>();
  ...
  int						substop = 0;
	...
	void distanceMatrixNumeric( List<Sequence> lseq, List<double> dmat, List<int> idxs, bool bootstrap, bool cantor, List<double> ent ) {		
		int len = lseq.length;
	...
	static Map<String,String>	complimentMap = new HashMap<String,String>();
	static {
	...
	Sequence( String id, String name, String sb, Map<String,Sequence> mseq ) {
	...
	String getProteinSequence( int start, int stop, int ori ) {
		...
		if( ori == -1 ) {
			...
			for( int i = stop-3; i > begin; i-=3 ) {
				...
				if( aa != null ) ret += i != stop-3 ? aa : (aa == "V" || aa == "L" ? "M" : aa);
				//else break;
			}
		} else {
			...
			for( int i = start-1; i < end; i+=3 ) {
				...
				if( aa != null ) ret += i != start-1 ? aa : (aa == "V" || aa == "L" ? "M" : aa);
				//else break;
			}
		}
