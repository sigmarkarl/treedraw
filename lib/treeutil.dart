...
class TreeUtil {
	...
	Node getParent( Node root, Set<String> leaveNames ) {
		...
		if( currentLeaveNames.length >= leaveNames.length ) {
			...
			if( currentLeaveNames == leaveNames ) return root;
			else {
	...
	String getSelectString( Node n, bool meta ) {
		...
		} else for( Node nn in n.getNodes() ) {
	...
	void invertSelectionRecursive( Node root ) {
		...
		if( root.getNodes() != null ) for( Node n in root.getNodes() ) {
	...
	List<List<double>> lms( List<double> distmat, List<String> corrInd, Node toptree ) {
	...
	Node neighborJoin( List<double> corrarr, List<String> corrInd, Node guideTree, bool rootTree ) {
	...
	void extractMetaRecursive( Node node, Map<String,Map<String,String>> mapmap, Set<String> collapset, bool collapse ) {
	...
	void init( String str, bool inverse, Set<String> include, Map<String,Map<String,String>> mapmap, bool collapse, Set<String> collapset, Map<String,String> colormap, bool clearParentNodes ) {
		//super();
		loc = 0;
	...
	void collapseTreeAdvanced( Node node, List<String> collapset, bool simple ) {
		if( node.nodes != null && node.nodes.length > 0 ) {
			...
			if( collapse ) {
				for( Node n in node.nodes ) {
					if( simple ) {
						...
						if( test == null ) {
							test = nname;
						} else if( test.length == 0 || nname.length == 0 || nname != test ) { //!(nname.contains(test) || test.contains(nname)) ) {
			...
			if( collapse && (collapset == null || collapset.contains(test)) ) {
				...
				else node.setName( test+";"+count.toString() );
			}
		}
	}
	...
	void replaceNames( Node node, Map<String,String> namesMap ) {
		List<Node> nodes = node.getNodes();
		if( nodes == null || nodes.length == 0 ) {
			if( namesMap.containsKey( node.getName() ) ) node.setName( namesMap[node.getName()] );
		} else {
			for( Node n in nodes ) {
					...
					if( split.length > 2 ) {
						...
						if( color.contains("rgb") ) {
							try {
								...
								node.color = "rgb("+r.toString()+","+g.toString()+","+b.toString()+")"; //new Color( r,g,b );
							} on Exception {
			...
			if( ci != -1 ) {
				...
				if( split.length > 2 ) {
					...
					try {
						...
						int g = int.parse( color.substring(2, 4), radix: 16 );
						int b = int.parse( color.substring(4, 6), radix: 16 );
						ret.color = "rgb("+r.toString()+","+g.toString()+","+b.toString()+")"; //new Color( r,g,b );
					} on Exception {
